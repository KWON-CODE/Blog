...
추상화(abstraction)는 공통 속성과 기능을 추출해 이름을 붙이는 과정으로, 객체지향 프로그래밍에서는 클래스를 정의



------------------래퍼 클래스 (Wrapper Class)-------------------------
이전 강의글Visit Website에서 우리는 자바의 자료형은 기본 타입(primitive type)과 참조 타입(reference type) 으로 나누어지고 각 특징에 대해 알아보았다.
